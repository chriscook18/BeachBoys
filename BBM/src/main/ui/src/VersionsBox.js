...
class VersionsBox extends React.Component {
      ...
      <Card style={{ width: '100%' }}>
        <Card.Body>
          ...
          {versionsError ? (
            <Row>
              <Col>
                <p>Failed to load versions: {versionsError.message}</p>
              </Col>
            </Row>
          ) : (
            [
                          ...
                          versions.map((version) => {
                            return (
                              <VersionDetails
                                ...
                                notes={version.notes}
                                duration={version.length}
                                youtube={version.youtube}
                                songperformer={songperformer}
                                appearsOn={version.appearsOn}
                              />
                            );
                          })}
                      </Tab.Content>
  ...
  fetchVersions() {
    ...
    fetch(
      constants.BASE_URL + `/recordings/` + this.props.recordingID + `/versions`
    )
...
class VersionDetails extends React.Component {
    ...
    return (
      <Tab.Pane eventKey={'#link' + tab} id={versionID}>
        {appearancesError ? (
          <Row>
            <Col>
              <p>Failed to load version details: {appearancesError.message}</p>
            </Col>
          </Row>
        ) : (
          <>
            {Number(duration) === 0 ? '' : 'Duration: ' + duration}{' '}
            <p>{notes}</p>
            {!youtube ? (
              ''
            ) : (
  ...
  fetchAppearances() {
    // eslint-disable-next-line no-undef
    fetch(
      constants.BASE_URL + `/version/` + this.props.versionID + `/appearson`
    )
      .then((response) => response.json())
      .then((data) =>
        this.setState({
...
class AppearanceDisplay extends React.Component {
  render() {
    ...
    return (
      <>
        {release.artist
          ? this.getArtistNameAppearanceFormat(songperformer, release.artist)
          : ''}{' '}
        {release.releasetype.italics ? (
          <i>{release.title}</i>
        ) : (
          [
            release.releasetype.quotes
              ? '"' + release.title + '"'
              : release.title,
          ]
        )}{' '}
        {release.unreleased ? <b>unreleased</b> : ''}{' '}
        {release.releasetype.description}{' '}
        {release.description === '' ? '' : ' (' + release.description + ')'} (
        {createDate(release.year, release.month, release.day)},{' '}
        {release.country}, {release.label}, {release.catalogNo}){' '}
        {appearance.firstappearance ? <b>First appearance</b> : ''}
      </>
    );
  }
